# 7.3. Основы и принцип работы Терраформ — Алексей Храпов
...
1. Создан бакет в облаке Яндекса, т.к. AWS по-прежнему не дает завершить регистрацию.
...
```bash
kad@g-deb-test:~/studing/devops-netology/terraform/07-terraform-03-basic$ terraform init
...
- Finding terraform-registry.storage.yandexcloud.net/yandex-cloud/yandex versions matching "~> 0.72"...
...
```bash
kad@g-deb-test:~/studing/devops-netology/terraform/07-terraform-03-basic$ terraform workspace new stage
Created and switched to workspace "stage"!

You're now on a new, empty workspace. Workspaces isolate their state,
so if you run "terraform plan" Terraform will not see any existing state
for this configuration.
kad@g-deb-test:~/studing/devops-netology/terraform/07-terraform-03-basic$ terraform workspace new prod
...
```bash
kad@g-deb-test:~/studing/devops-netology/terraform/07-terraform-03-basic$ terraform workspace list
  default
* prod
  stage
```
...
```
Конфигурации доступны по [ссылке](https://github.com/AlexeyKhrapov/devops-netology/tree/main/terraform/07-terraform-03-basic).

Отработка бэкэнда:
![backend-yc](./src/bucket.png)